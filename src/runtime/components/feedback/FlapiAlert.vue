<template>
  <div v-if="message" :class="alertClasses" role="alert" class="flex items-center rounded-lg border p-4">
    <FlapiIcon v-if="computedIcon" :name="computedIcon" class="-mb-1 mr-3" :class="iconColor" mode="stroke" />
...
import type { FlapiAlertProps, FlapiAlertVariant } from '#/core'

/**
 * Type definitions for the FlapiAlert component props
 * @type {FlapiAlertProps}
 ...
 */
const props: FlapiAlertProps = defineProps({
  message: {
    type: String,
    required: true,
  },
  ...
  variant: {
    type: String as PropType<FlapiAlertVariant>,
    default: 'info',
  },
})
